const dbQuery = (connection, query, data) => {
  ...
  const rangeDateQueries = (urlQueries, prefix = '') => {
    const { startDate, endDate, filter } = urlQueries
    if(!startDate, !endDate) return '';
    const query = ` ${prefix}createdAt >= '${startDate} 00:00:00' and ${prefix}createdAt <= '${endDate} 23:59:59'`;
    let result = !filter ? ` WHERE ${query}` : ` AND ${query}`;
    return result
  }
